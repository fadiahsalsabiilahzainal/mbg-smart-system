...
export default function HomeDashboard({ isAdmin, logEntries }: HomeDashboardProps) {
  
  // Logika Kalkulasi Jumlah Item Gizi
  let totalItemSegar = 0;
  let totalItemBasi = 0;

  logEntries.forEach((log) => {
    if (log.jenis_makanan) {
      const items = log.jenis_makanan.split("\n");
      items.forEach((item: string) => {
        if (item.includes("SEGAR")) totalItemSegar++;
        if (item.includes("BASI")) totalItemBasi++;
      });
    }
  });
      ...
      {isAdmin ? (
        <div style={{ textAlign: "center", padding: "1rem 0", width: "100%" }}>
          <div style={{ marginBottom: "2rem", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
            ...
            <p style={{ fontSize: isMobile ? "0.85rem" : "1rem", color: "#153759", margin: "0 0 20px 0", maxWidth: "100%", lineHeight: "1.5", fontWeight: "500" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem", width: "100%", marginBottom: "2rem" }}>
            <div className="profil-card" style={{ padding: isMobile ? "1.5rem" : "2.5rem 1.5rem", borderRadius: "12px", borderBottom: "4px solid #2ecc71", textAlign: "center", backgroundColor: "white", boxShadow: "0 4px 6px rgba(0,0,0,0.05)" }}>
              <p style={{ fontSize: isMobile ? "0.85rem" : "1rem", color: "var(--clr-gray-500)", fontWeight: "700", marginBottom: "0.5rem" }}>KUALITAS SEGAR</p>
              <h3 style={{ fontSize: isMobile ? "2.5rem" : "3.5rem", color: "#4cbb17", margin: "0.5rem 0", fontWeight: "bold" }}>{totalItemSegar}</h3>
              <p style={{ fontSize: isMobile ? "0.75rem" : "0.95rem", color: "var(--clr-gray-400)", fontWeight: "600" }}>Item layak konsumsi</p>
            </div>
            
            <div className="profil-card" style={{ padding: isMobile ? "1.5rem" : "2.5rem 1.5rem", borderRadius: "12px", borderBottom: "4px solid #e74c3c", textAlign: "center", backgroundColor: "white", boxShadow: "0 4px 6px rgba(0,0,0,0.05)" }}>
              <p style={{ fontSize: isMobile ? "0.85rem" : "1rem", color: "var(--clr-gray-500)", fontWeight: "700", marginBottom: "0.5rem" }}>KUALITAS BASI</p>
              <h3 style={{ fontSize: isMobile ? "2.5rem" : "3.5rem", color: "#e74c3c", margin: "0.5rem 0", fontWeight: "bold" }}>{totalItemBasi}</h3>
              <p style={{ fontSize: isMobile ? "0.75rem" : "0.95rem", color: "var(--clr-gray-400)", fontWeight: "600" }}>Item tidak layak</p>
            </div>
            
            <div className="profil-card" style={{ padding: isMobile ? "1.5rem" : "2.5rem 1.5rem", borderRadius: "12px", borderBottom: "4px solid #7ea69f", textAlign: "center", backgroundColor: "white", boxShadow: "0 4px 6px rgba(0,0,0,0.05)" }}>
              <p style={{ fontSize: isMobile ? "0.85rem" : "1rem", color: "var(--clr-gray-500)", fontWeight: "700", marginBottom: "0.5rem" }}>TOTAL PEMERIKSAAN</p>
              <h3 style={{ fontSize: isMobile ? "2.5rem" : "3.5rem", color: "#153759", margin: "0.5rem 0", fontWeight: "bold" }}>{logEntries.length}</h3>
              <p style={{ fontSize: isMobile ? "0.75rem" : "0.95rem", color: "var(--clr-gray-400)", fontWeight: "600" }}>Laporan nampan masuk</p>
            </div>
          </div>
        </div>
      ) : (
        <div style={{ textAlign: "center", padding: "1rem 0", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <p style={{ color: "#334155", maxWidth: "100%", lineHeight: "1.8", fontSize: isMobile ? "1rem" : "1.1rem", marginBottom: "2.5rem", fontWeight: "500" }}>
          ...
          <div style={{ width: "100%", maxWidth: "100%", padding: "0", textAlign: "left", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ background: "rgba(232, 244, 248, 0.95)", borderLeft: "5px solid var(--clr-teal)", padding: isMobile ? "1.2rem 1.5rem" : "1.5rem 2rem", borderRadius: "12px", width: "100%", marginBottom: "2rem", boxShadow: "0 4px 15px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ borderTop: "1px solid rgba(21, 55, 89, 0.1)", paddingTop: "12px", marginTop: "12px", fontSize: isMobile ? "0.82rem" : "0.95rem", color: "#475569", lineHeight: "1.6", fontWeight: "500" }}>
              ...
              <table className="log-table" style={{ width: "100%", minWidth: "600px", borderCollapse: "collapse", fontSize: isMobile ? "0.85rem" : "1rem" }}>
                <thead>
                  <tr style={{ textAlign: "center", backgroundColor: "#153759", color: "white" }}>
                    ...
                    <th style={{ padding: isMobile ? "12px" : "14px 12px", fontSize: isMobile ? "0.85rem" : "1.05rem" }}>Kalori(kkal)</th>
                    <th style={{ padding: isMobile ? "12px" : "14px 12px", fontSize: isMobile ? "0.85rem" : "1.05rem" }}>Protein (g)</th>
                    <th style={{ padding: isMobile ? "12px" : "14px 12px", fontSize: isMobile ? "0.85rem" : "1.05rem" }}>Lemak(g)</th>
                    <th style={{ padding: isMobile ? "12px" : "14px 12px", fontSize: isMobile ? "0.85rem" : "1.05rem" }}>Karbo (g)</th>
                    <th style={{ padding: isMobile ? "12px" : "14px 12px", fontSize: isMobile ? "0.85rem" : "1.05rem" }}>BDD (%)</th>
                  </tr>
                </thead>
                <tbody>
