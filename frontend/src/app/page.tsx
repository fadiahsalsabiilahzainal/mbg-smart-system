...
export default function LoginPage() {
  ...
  const [roleDaftar, setRoleDaftar] = useState<"Pegawai SPPG" | "Guru">(
    "Pegawai SPPG",
  );
  ...
  useEffect(() => {
    if (provinsiTerpilih.id) {
      fetch(
        `https://www.emsifa.com/api-wilayah-indonesia/api/regencies/${provinsiTerpilih.id}.json`,
      )
        .then((res) => res.json())
  ...
  const [resetMethod, setResetMethod] = useState<"whatsapp" | "email">(
    "whatsapp",
  );
  ...
  const showCustomPopup = (
    title: string,
    message: string,
    icon: string = "/assets/checklist.png",
  ...
  const PasswordToggle = ({
    ...
    >
      {isVisible ? (
        <img
          src="/assets/icon-mata-terbuka.png"
          alt="Show Password"
          style={{ width: "24px", height: "24px", objectFit: "contain" }}
        />
      ) : (
        <img
          src="/assets/icon-mata-tertutup.png"
          alt="Hide Password"
          style={{ width: "24px", height: "24px", objectFit: "contain" }}
        />
      )}
  ...
  const handleResetRequest = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      resetMethod === "whatsapp" &&
      (resetPhone.startsWith("0") || !resetPhone.startsWith("62"))
    ) {
    ...
    try {
      console.log(
        `Mencari data menggunakan metode: ${resetMethod} | Input: ${resetPhone.trim()}`,
      );
      ...
      console.log("Respon dari Supabase:", { user, error });

      if (error || !user)
        throw new Error(
          resetMethod === "whatsapp"
            ? "Nomor HP tidak ditemukan di sistem!"
            : "Alamat email tidak ditemukan di sistem!",
        );
      ...
      if (resetMethod === "whatsapp") {
        ...
        if (!resData.status)
          throw new Error("Gagal mengirim WA. Pastikan token Fonnte aktif.");
      } else {
        const waktuExpired = new Date();
        waktuExpired.setMinutes(waktuExpired.getMinutes() + 5);
        const jamExpired =
          waktuExpired.toLocaleTimeString("id-ID", {
            hour: "2-digit",
            minute: "2-digit",
          }) + " WIB";
      ...
      setSuccessMsg(
        `Kode OTP berhasil dikirim ke ${resetMethod.toUpperCase()}`,
      );
      setResetStep(2);
    } catch (err: any) {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPassword) {
      setErrorMsg("Mohon masukkan sandi baru Anda!");
      return;
    }
    if (newPassword.length < 8) {
      setErrorMsg("Sandi minimal 8 karakter!");
      return;
    }
    setIsLoading(true);
    try {
      ...
      await supabase
        .from("users")
        .update({ password: hashedPassword, otp_code: null, otp_expiry: null })
        .eq("id", targetUser.id);
      showCustomPopup(
        "Sandi Diperbarui!",
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      setErrorMsg("Harap isi semua kolom dengan lengkap!");
      return;
    }
    ...
    if (roleDaftar === "Guru") {
      if (!provinsiTerpilih.name || !kotaTerpilih.name || !sekolah) {
        setErrorMsg(
          "Harap isi Provinsi, Kota, and Nama Sekolah dengan lengkap!",
        );
        return;
      }
    ...
    if (
      ...
      setErrorMsg(
        "Sandi harus minimal 8 karakter & mengandung huruf serta angka!",
      );
      return;
    }
    ...
    try {
      const { data: existingUser } = await supabase
        ...
        .or(
          `username.eq.${username.toLowerCase()},no_hp.eq.${noHp},email.eq.${email.toLowerCase()}`,
        );

      if (existingUser && existingUser.length > 0) {
        if (existingUser.some((u) => u.username === username.toLowerCase()))
          throw new Error("Username sudah digunakan!");
        if (existingUser.some((u) => u.no_hp === noHp))
          throw new Error("Nomor WhatsApp ini sudah terdaftar!");
        if (existingUser.some((u) => u.email === email.toLowerCase()))
          throw new Error("Alamat Email ini sudah digunakan!");
      }
    ...
    <>
      ...
      >
        <div
          ...
          style={{ top: "8%", left: "5%", width: "90px", position: "absolute" }}
        >
          <img
            src="/assets/tempe.png"
            alt="Tempe"
      ...
      >
        <div
          ...
          style={{
            ...
            borderRadius: "24px",
            padding: isMobile ? "40px 24px" : "60px 55px",
            ...
            <>
              ...
              <form onSubmit={handleLogin} noValidate>
                ...
                >
                  ...
                  >
                    <img
                      src="/assets/icon-user-login.png"
                      alt="User Icon"
                      style={{
                        width: "26px",
                        height: "26px",
                ...
                >
                  ...
                  >
                    <PasswordToggle
                      isVisible={showPasswordLogin}
                      setIsVisible={setShowPasswordLogin}
                    />
                  </div>
                </div>
              ...
              >
                ...
                >
                  Untuk kendala akses, hubungi administrator.
                </p>
              ...
              >
                <p style={{ margin: "0", fontWeight: "500" }}>
                  ...
                  <strong style={{ color: "var(--clr-navy)" }}>
                    FYM Project
                  </strong>
                </p>
                <p
              ...
              <form onSubmit={handleRegister} noValidate>
                <div style={{ width: "100%", marginBottom: "14px" }}>
                  <div style={{ display: "flex", gap: "10px" }}>
                    <button
                      type="button"
                      onClick={() => setRoleDaftar("Pegawai SPPG")}
                      style={{
                        flex: 1,
                        padding: "10px",
                        borderRadius: "8px",
                    ...
                    <button
                      type="button"
                      onClick={() => setRoleDaftar("Guru")}
                      style={{
                        flex: 1,
                        padding: "10px",
                        borderRadius: "8px",
                        background:
                          roleDaftar === "Guru" ? "#153759" : "transparent",
                        color:
                          roleDaftar === "Guru"
                            ? "#ffffff"
                            : "var(--clr-gray-500)",
                        border:
                          roleDaftar === "Guru" ? "none" : "1px solid #7da2a9",
                        outline: "none",
                        cursor: "pointer",
                        fontWeight: "bold",
                        transition: "0.2s",
                ...
                <div
                  style={{
                    position: "relative",
                    width: "100%",
                    marginBottom: noHp.startsWith("0") ? "4px" : "16px",
                  }}
                >
                  <input
                    type="tel"
                ...
                {roleDaftar === "Pegawai SPPG" ? (
                  ...
                  >
                    <input
                      type="text"
                      placeholder="Lokasi SPPG (Cont: Dahyeukolot 1, Bojongsoang)"
                      value={lokasi}
                  ...
                  >
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        onBlur={() =>
                          setTimeout(() => setIsProvinsiOpen(false), 200)
                        }
                        style={{
                          width: "100%",
                          padding: isMobile ? "12px" : "16px",
                        ...
                        >
                          {daftarProvinsi
                            .filter((p) =>
                              p.name
                                .toLowerCase()
                                .includes(searchProvinsi.toLowerCase()),
                            )
                            .map((p) => (
                              <div
                                key={p.id}
                    ...
                    <div style={{ position: "relative" }}>
                      <input
                        ...
                        onBlur={() =>
                          setTimeout(() => setIsKotaOpen(false), 200)
                        }
                        style={{
                          width: "100%",
                          padding: isMobile ? "12px" : "16px",
                          ...
                          outline: "none",
                          fontSize: isMobile ? "0.95rem" : "1.05rem",
                          background: !provinsiTerpilih.id
                            ? "#f1f5f9"
                            : "white",
                          boxSizing: "border-box",
                        }}
                        ...
                        >
                          {daftarKota
                            .filter((k) =>
                              k.name
                                .toLowerCase()
                                .includes(searchKota.toLowerCase()),
                            )
                            .map((k) => (
                              <div
                                key={k.id}
                                ...
                                onMouseDown={() => {
                                  setKotaTerpilih({ id: k.id, name: k.name });
                                  setSearchKota(k.name);
                                  setIsKotaOpen(false);
                                }}
                              >
                                {k.name}
                ...
                >
                  ...
                  >
                    <PasswordToggle
                      isVisible={showPasswordRegister}
                      setIsVisible={setShowPasswordRegister}
                    />
                  </div>
                </div>
                <p
                ...
                >
                  ...
                  >
                    <PasswordToggle
                      isVisible={showConfirmPassRegister}
                      setIsVisible={setShowConfirmPassRegister}
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: isMobile ? "1rem" : "1.1rem",
                    cursor: "pointer",
                    boxShadow: "0 4px 10px rgba(21, 55, 89, 0.15)",
                  }}
                >
                      ...
                      style={{
                        ...
                        border:
                          resetMethod === "whatsapp"
                            ...
                            : "1px solid #cbd5e1",
                        background:
                          resetMethod === "whatsapp" ? "#e0f2fe" : "white",
                        color:
                          resetMethod === "whatsapp"
                            ? "#0369a1"
                            : "var(--clr-navy)",
                      ...
                      style={{
                        ...
                        border:
                          resetMethod === "email"
                            ...
                            : "1px solid #cbd5e1",
                        background:
                          resetMethod === "email" ? "#e0f2fe" : "white",
                        color:
                          resetMethod === "email"
                            ? "#0369a1"
                            : "var(--clr-navy)",
